...
const PanToInsuranceDetailsSearch: React.FC = () => {
  ...
  useEffect(() => {
    if (apis && officer) {
      const api = apis.find(a => a.name.toLowerCase().includes('pan to insurance details'));
  ...
  const handlePanToInsuranceSearch = async () => {
    if (!panNumber.trim() || !/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(panNumber)) {
      toast.error('Please enter a valid PAN number (e.g., BUXPB0807N)');
      return;
    }

    const enabledAPIs = getOfficerEnabledAPIs(officer.id);
    const insuranceAPI = enabledAPIs.find(api =>
      api.name.toLowerCase().includes('pan to insurance details') && api.key_status === 'Active'
    );

    if (!insuranceAPI) {
      setSearchError('PAN to Insurance Details search service is currently unavailable');
      toast.error('PAN to Insurance Details search service is currently unavailable');
      return;
    }

    const creditCost = insuranceAPI.default_credit_charge || 5.00;
    ...
    try {
      const response = await fetch('/api/signzy/api/v3/global-api/PanToInsuranceDetails', {
        method: 'POST',
        headers: {
          'Authorization': insuranceAPI.api_key,
      ...
      if (data.result?.[0]?.result) {
        ...
        throw new Error('No Insurance data found');
      }
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `PAN to Insurance Details Search for PAN ${panNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'PAN to Insurance Details Search',
          input_data: panNumber,
          source: 'Signzy API',
          result_summary: `Insurance details found for PAN: ${data.result?.data?.[0]?.policyNumber || 'N/A'}`,
      ...
      toast.success('Insurance details retrieved successfully!');
    } catch (error: any) {
      console.error('PAN to Insurance Search Error:', error);
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
            PAN to Insurance Details Search
          </h3>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder="Enter PAN number (e.g., BUXPB0807N)"
        ...
        <div className="flex items-end">
          <button
            onClick={handlePanToInsuranceSearch}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('pan to insurance details'))?.default_credit_charge || 5.00} credits per query.
      </p>
      ...
      {searchError && (
        <div className={`p-4 rounded-lg border flex items-center space-x-3 ${isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'} mb-6`}>
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              {searchError}
              {searchError === 'PAN to Insurance Details search service is currently unavailable' ? (
            ...
            {expandedSections.details && (
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-muted-graphite border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                {Array.isArray(searchResults) ? (
                  searchResults.map((result: any, index: number) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4">
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Policy Number:</span>
                        <div className="flex items-center space-x-2">
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {result.policyNumber || 'N/A'}
                          </span>
                          {result.policyNumber && (
                            <button
                              onClick={() => copyToClipboard(result.policyNumber)}
                      ...
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Insurer Name:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.insurerName || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Coverage Amount:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.coverageAmount || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Start Date:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  ...
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>No Insurance information available.</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pan-to-insurance-${panNumber}-${Date.now()}.json`;
                link.click();
...
export default PanToInsuranceDetailsSearch;