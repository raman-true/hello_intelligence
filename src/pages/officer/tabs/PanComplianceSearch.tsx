...
const PanComplianceSearch: React.FC = () => {
  ...
  useEffect(() => {
    if (apis && officer) {
      const api = apis.find(a => a.name === 'PAN Compliance 206 Individual Search');
  ...
  const handlePanComplianceSearch = async () => {
    if (!panNumber.trim() || !/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(panNumber)) {
      toast.error('Please enter a valid PAN number (e.g., BYZPS6730F)');
      return;
    }

    const enabledAPIs = getOfficerEnabledAPIs(officer.id);
    const complianceAPI = enabledAPIs.find(api =>
      api.name === 'PAN Compliance 206 Individual Search' && api.key_status === 'Active'
    );

    if (!complianceAPI) {
      setSearchError('PAN Compliance search service is currently unavailable');
      toast.error('PAN Compliance search service is currently unavailable');
      return;
    }

    const creditCost = complianceAPI.default_credit_charge || 5.00;
    ...
    try {
      const response = await fetch('/api/signzy/api/v3/pan/compliance-206-individual-search', {
        method: 'POST',
        headers: {
          'Authorization': complianceAPI.api_key,
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `PAN Compliance Search for PAN ${panNumber}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'PAN Compliance Search',
          input_data: panNumber,
          source: 'Signzy API',
          result_summary: `Compliance details found for PAN: ${data.number || 'N/A'}`,
          full_result: data,
      ...
      toast.success('Compliance details retrieved successfully!');
    } catch (error: any) {
      console.error('PAN Compliance Search Error:', error);
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder="Enter PAN number (e.g., BYZPS6730F)"
        ...
        <div className="flex items-end">
          <button
            onClick={handlePanComplianceSearch}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name === 'PAN Compliance 206 Individual Search')?.default_credit_charge || 5.00} credits per query.
      </p>
      ...
      {searchError && (
        <div className={`p-4 rounded-lg border flex items-center space-x-3 ${isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'} mb-6`}>
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              {searchError}
              {searchError === 'PAN Compliance search service is currently unavailable' ? (
                ...
                {Array.isArray(searchResults) ? (
                  searchResults.map((result: any, index: number) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4">
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Compliance Status:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.compliant || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.entityName || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Masked Name:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.maskedName || maskedName || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Allotment Date:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.panAllotmentDate || 'N/A'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Aadhaar Link Status:</span>
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {result.panAadhaarLinkStatus || 'N/A'}
                        </span>
                      </div>
                    </div>
                  ))
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          {searchResults.number || panNumber || 'N/A'}
                        </span>
                        {searchResults.number && (
                          <button
                            onClick={() => copyToClipboard(searchResults.number)}
                            className="p-1 text-cyber-teal hover:text-electric-blue transition-colors"
                            title="Copy PAN Number"
                          >
                            <Copy className="w-4 h-4" />
                    ...
                    <div className="flex justify-between items-center">
                      <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Name:</span>
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults.entityName || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Masked Name:</span>
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults.maskedName || maskedName || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Allotment Date:</span>
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults.panAllotmentDate || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Aadhaar Link Status:</span>
                      <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {searchResults.panAadhaarLinkStatus || 'N/A'}
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pan-compliance-${panNumber}-${Date.now()}.json`;
                link.click();
...
export default PanComplianceSearch;