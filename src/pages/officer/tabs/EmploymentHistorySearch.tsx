...
const EmploymentHistorySearch: React.FC = () => {
  ...
  useEffect(() => {
    if (apis && officer) {
      const api = apis.find(a => a.name === 'Employment History Search');
  ...
  const handleEmploymentHistorySearch = async () => {
    if (!userId.trim()) {
    ...
    const employmentApi = enabledAPIs.find(api =>
      api.name === 'Employment History Search' && api.key_status === 'Active'
    );
    ...
    const creditCost = employmentApi.default_credit_charge || 5.00;
    ...
    try {
      const response = await fetch('/api/signzy/api/v3/underwriting/fetch-employment-history-v2', {
        method: 'POST',
        headers: {
          'Authorization': employmentApi.api_key,
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Employment History Fetch for userId ${userId}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Employment History Search',
          input_data: userId,
          source: 'Signzy API',
          result_summary: `Employment history result for userId ${userId}: ${data.result?.length || '0'} records found`,
      ...
      toast.success('Employment history retrieved successfully!');
    } catch (error: any) {
      console.error('Employment History Search Error:', error);
    ...
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="flex items-end">
          <button
            onClick={handleEmploymentHistorySearch}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name === 'Employment History Search')?.default_credit_charge || 5.00} credits per query.
      </p>
            ...
            {expandedSections.details && (
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-muted-graphite border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="space-y-4 text-sm">
                  {Array.isArray(searchResults.employmentRecords) && searchResults.employmentRecords.length > 0 ? (
                    searchResults.employmentRecords.map((record: any, index: number) => (
                      <div key={index} className="border-b pb-2 last:border-b-0">
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Company:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {record.company || 'N/A'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Title:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {record.title || 'N/A'}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>End Date:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {record.endDate || 'N/A'}
                          </span>
                        </div>
...
export default EmploymentHistorySearch;