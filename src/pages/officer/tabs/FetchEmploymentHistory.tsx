...
const FetchEmploymentHistory: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [searchMethod, setSearchMethod] = useState<'uan' | 'pan' | 'mobile' | 'panWithExtras'>('uan');
  ...
  useEffect(() => {
    if (apis && officer) {
      const api = apis.find(a => a.name === 'Fetch Employment History');
  ...
  const handleFetchEmploymentHistory = async () => {
    let body: any = {};
    let isValid = false;
    ...
    switch (searchMethod) {
      case 'uan':
        if (uan.trim()) {
          ...
          toast.error('UAN is required for this search method');
        }
        break;
      case 'pan':
        if (pan.trim()) {
          ...
          toast.error('PAN is required for this search method');
        }
        break;
      case 'mobile':
        if (mobile.trim()) {
          ...
          toast.error('Mobile number is required for this search method');
        }
        break;
      case 'panWithExtras':
        if (!pan.trim()) {
        ...
        if (employeeName.trim()) {
          body.employeeName = employeeName.trim();
          isValid = true;
        } else {
          toast.error('Employee name is required when using PAN with extras');
        }
        break;
    }
    ...
    const historyApi = enabledAPIs.find(api =>
      api.name === 'Fetch Employment History' && api.key_status === 'Active'
    );

    if (!historyApi) {
      setSearchError('Employment history service is currently unavailable');
      toast.error('Employment history service is currently unavailable');
      return;
    }

    const creditCost = historyApi.default_credit_charge || 5.00;
    ...
    try {
      const response = await fetch('/api/signzy/api/v3/underwriting/fetch-employment-history-v2', {
        method: 'POST',
        headers: {
          'Authorization': historyApi.api_key,
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Employment History Fetch for ${body.uan || body.pan || body.mobile}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Fetch Employment History',
          input_data: body,
          source: 'Signzy API',
          result_summary: `Employment history fetched: ${data.result?.employmentHistory?.length || 0} records`,
      ...
      toast.success('Employment history fetched successfully!');
    } catch (error: any) {
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="mb-4">
        ...
        >
          <option value="uan">UAN Only</option>
          <option value="pan">PAN Only</option>
          <option value="mobile">Mobile Only</option>
          <option value="panWithExtras">PAN with Extras</option>
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        {(searchMethod === 'pan' || searchMethod === 'panWithExtras') && (
          ...
          <>
            ...
            <div className="md:col-span-1">
              ...
              <input
                ...
                placeholder="Enter DOB (e.g., 1990-05-15)"
                className={`w-full px-4 py-3 border border-cyber-teal/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyber-teal ${
                  isDark ? 'bg-crisp-black text-white placeholder-gray-500' : 'bg-white text-gray-900 placeholder-gray-400'
                }`}
              />
        ...
        <div className="md:col-span-3 flex items-end">
          <button
            onClick={handleFetchEmploymentHistory}
            disabled={isSearching || !((searchMethod === 'uan' && uan.trim()) || (searchMethod === 'pan' && pan.trim()) || (searchMethod === 'mobile' && mobile.trim()) || (searchMethod === 'panWithExtras' && pan.trim() && employeeName.trim()))}
          ...
          >
            {isSearching ? (
              ...
              <>
                ...
                <span>Fetch Employment History</span>
              </>
            )}
          </button>
        </div>
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name === 'Fetch Employment History')?.default_credit_charge || 5.00} credits per query.
      </p>
      ...
      {searchError && (
        <div className={`p-4 rounded-lg border flex items-center space-x-3 ${isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'} mb-6`}>
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              {searchError}
              {searchError === 'Employment history service is currently unavailable' ? (
            ...
            {expandedSections.details && (
              <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-muted-graphite border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
                <div className="space-y-4 text-sm">
                  {searchResults.employmentHistory && searchResults.employmentHistory.length > 0 && (
                    <>
                      <h6 className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Employment History</h6>
                      {searchResults.employmentHistory.map((record: any, index: number) => (
                        <div key={index} className="border-b pb-2 last:border-b-0">
                          ...
                          <div className="flex justify-between items-center">
                            <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Tenure (months):</span>
                            <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                              {record.tenureOfEmployment || 'N/A'}
                            </span>
                          </div>
                        </div>
                      ))}
                  ...
                  {searchResults.recentEmployerData && (
                    <div>
                      <h6 className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Recent Employer</h6>
                      <div className="border-b pb-2">
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Establishment Name:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {searchResults.recentEmployerData.establishmentName || 'N/A'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Date of Joining:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {searchResults.recentEmployerData.dateOfJoining || 'N/A'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Date of Exit:</span>
                          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {searchResults.recentEmployerData.dateOfExit || 'N/A'}
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `employment-history-${Date.now()}.json`;
                link.click();
...
export default FetchEmploymentHistory;