...
const MobileToName: React.FC = () => {
  ...
  useEffect(() => {
    if (apis && officer) {
      setIsLoading(false);
      if (!accessToken) {
        const api = apis.find(a => a.name.toLowerCase().includes('mobile to name'));
        if (api && api.api_key) {
  ...
  const handleSearch = async () => {
    ...
    const deepvueAPI = enabledAPIs.find(api => api.name.toLowerCase().includes('mobile to name') && api.key_status === 'Active');
    if (!deepvueAPI) {
      setSearchError('Mobile to Name API not configured or inactive');
      toast.error('Mobile to Name API not configured or inactive');
      return;
    }
    ...
    try {
      const response = await fetch(`/api/deepvue/v1/mobile-intelligence/mobile-to-name?mobile_number=${encodeURIComponent(mobileNumber.trim())}`, {
        method: 'GET',
        headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      });
      if (!response.ok) throw new Error(`API request failed: ${response.status} ${await response.text()}`);
      const data: MobileToNameResult = await response.json();
      ...
      if (addTransaction) await addTransaction({
        ...
        credits: creditCost, payment_mode: 'Query Usage', remarks: `Mobile to Name for ${mobileNumber}`,
      });
      if (addQuery) await addQuery({
        officer_id: officer.id, officer_name: officer.name || 'Unknown', type: 'PRO',
        category: 'Mobile to Name', input_data: `Mobile Number: ${mobileNumber}`,
        source: 'Deepvue', result_summary: `Name fetched: ${data.message || 'Success'}`,
        full_result: data, credits_used: creditCost, status: data.code === 200 ? 'Success' : 'Failed',
      });

      toast.success(data.code === 200 ? 'Name retrieved successfully!' : `Search failed: ${data.message || 'Unknown error'}`);
    } catch (error) {
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3"><Phone className="w-6 h-6 text-electric-blue" /><h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Mobile to Name</h3></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end"><button onClick={handleSearch} disabled={isSearching || !mobileNumber.trim() || !accessToken}
          ...
          {isSearching ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /><span>Searching...</span></> :
            <><Search className="w-4 h-4" /><span>Search Name</span></>}
        </button></div>
      </div>
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('mobile to name'))?.default_credit_charge || 5.00} credits per query.
      </p>
      {searchError && (
      ...
      {searchResults && searchResults.data && (
        <div className={`p-6 rounded-lg border ${isDark ? 'bg-green-500/10 border-green-500/30' : 'bg-green-50 border-green-200'}`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3"><CheckCircle className="w-5 h-5 text-green-400" /><h4 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Name Details</h4></div>
            <div className="flex items-center space-x-2"><span className={`text-xs px-2 py-1 rounded ${isDark ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-600'}`}>Verified 8/2/2025, 1:00 AM</span></div>
          </div>
          <div className="mb-6"><button onClick={() => toggleSection('details')}
            ...
            <h5 className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>Name Information</h5>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button onClick={() => {
              ...
              link.download = `mobile-to-name-${mobileNumber}-${Date.now()}.json`; link.click(); URL.revokeObjectURL(url);
