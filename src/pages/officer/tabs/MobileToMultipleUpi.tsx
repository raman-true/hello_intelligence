...
interface MobileToMultipleUpiResult {
  code: number;
...
const MobileToMultipleUpi: React.FC = () => {
  const { isDark } = useTheme();
  const { officer, updateOfficerState } = useOfficerAuth();
  const { apis, addQuery, addTransaction, getOfficerEnabledAPIs } = useSupabaseData();
  const [mobileNumber, setMobileNumber] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchResults, setSearchResults] = useState<MobileToMultipleUpiResult | null>(null);
  ...
  useEffect(() => {
    if (apis && officer) {
      setIsLoading(false);
      if (!accessToken) {
        const api = apis.find(a => a.name.toLowerCase().includes('mobile to multiple upi'));
        if (api && api.api_key) {
  ...
  const handleSearch = async () => {
    ...
    const deepvueAPI = enabledAPIs.find(api => api.name.toLowerCase().includes('mobile to multiple upi') && api.key_status === 'Active');
    if (!deepvueAPI) {
      setSearchError('Mobile to Multiple UPI API not configured or inactive');
      toast.error('Mobile to Multiple UPI API not configured or inactive');
      return;
    }
    ...
    try {
      const response = await fetch(`/api/deepvue/v1/mobile-intelligence/mobile-to-multiple-upi?mobile_number=${encodeURIComponent(mobileNumber.trim())}`, {
        method: 'GET',
        headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      });
      if (!response.ok) throw new Error(`API request failed: ${response.status} ${await response.text()}`);
      const data: MobileToMultipleUpiResult = await response.json();
      ...
      if (addTransaction) await addTransaction({
        ...
        credits: creditCost, payment_mode: 'Query Usage', remarks: `Mobile to Multiple UPI for ${mobileNumber}`,
      });
      if (addQuery) await addQuery({
        officer_id: officer.id, officer_name: officer.name || 'Unknown', type: 'PRO',
        category: 'Mobile to Multiple UPI', input_data: `Mobile Number: ${mobileNumber}`,
        source: 'Deepvue', result_summary: `Multiple UPI fetched: ${data.message || 'Success'}`,
        full_result: data, credits_used: creditCost, status: data.code === 200 ? 'Success' : 'Failed',
      });

      toast.success(data.code === 200 ? 'Multiple UPI details retrieved successfully!' : `Search failed: ${data.message || 'No VPA linked'}`);
    } catch (error) {
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3"><Phone className="w-6 h-6 text-electric-blue" /><h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Mobile to Multiple UPI</h3></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-end"><button onClick={handleSearch} disabled={isSearching || !mobileNumber.trim() || !accessToken}
          ...
          {isSearching ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /><span>Searching...</span></> :
            <><Search className="w-4 h-4" /><span>Search Multiple UPI</span></>}
        </button></div>
      </div>
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name.toLowerCase().includes('mobile to multiple upi'))?.default_credit_charge || 5.00} credits per query.
      </p>
      {searchError && (
      ...
      {searchResults && searchResults.data && (
        <div className={`p-6 rounded-lg border ${isDark ? 'bg-green-500/10 border-green-500/30' : 'bg-green-50 border-green-200'}`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3"><CheckCircle className="w-5 h-5 text-green-400" /><h4 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Multiple UPI Details</h4></div>
            <div className="flex items-center space-x-2"><span className={`text-xs px-2 py-1 rounded ${isDark ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-600'}`}>Verified 8/2/2025, 12:10 AM</span></div>
          </div>
          <div className="mb-6"><button onClick={() => toggleSection('details')}
            ...
            <h5 className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>UPI Information</h5>
            ...
            <div className={`p-4 mt-2 rounded-lg border ${isDark ? 'bg-muted-graphite border-cyber-teal/10' : 'bg-white border-gray-200'}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div className="flex justify-between items-center"><span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Name at Bank:</span>
                  <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{searchResults.data.name_at_bank || 'N/A'}</span></div>
                <div className="flex justify-between items-center"><span className={isDark ? 'text-gray-400' : 'text-gray-600'}>UPI IDs:</span>
                  <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{(searchResults.data.vpas || []).join(', ') || 'N/A'}</span></div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button onClick={() => {
              ...
              link.download = `mobile-to-multiple-upi-${mobileNumber}-${Date.now()}.json`; link.click(); URL.revokeObjectURL(url);
...
export default MobileToMultipleUpi;