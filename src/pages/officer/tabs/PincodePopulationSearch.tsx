...
const PincodePopulationSearch: React.FC = () => {
  ...
  useEffect(() => {
    if (apis && officer) {
      const api = apis.find(a => a.name === 'Pincode Population Search');
  ...
  const handlePincodeSearch = async () => {
    ...
    const pincodeApi = enabledAPIs.find(api =>
      api.name === 'Pincode Population Search' && api.key_status === 'Active'
    );

    if (!pincodeApi) {
      setSearchError('Pincode search service is currently unavailable');
      toast.error('Pincode search service is currently unavailable');
      return;
    }

    const creditCost = pincodeApi.default_credit_charge || 5.00;
    ...
    try {
      const response = await fetch('/api/signzy/api/v3/global/pincodeDetails/pincodeToPopulation', {
        method: 'POST',
        headers: {
          'Authorization': pincodeApi.api_key,
      ...
      if (addTransaction) {
        await addTransaction({
          ...
          remarks: `Pincode Population Search for ${pincode}`,
        });
      }
      ...
      if (addQuery) {
        await addQuery({
          ...
          officer_name: officer.name || 'Unknown',
          type: 'PRO',
          category: 'Pincode Population Search',
          input_data: pincode,
          source: 'Signzy API',
          result_summary: `Population details found for pincode ${pincode}: ${data.result.data.message || 'N/A'}`,
          full_result: data.result.data,
          credits_used: creditCost,
          status: 'Success'
        });
      }

      toast.success('Population details retrieved successfully!');
    } catch (error: any) {
      console.error('Pincode Population Search Error:', error);
  ...
  return (
    <div className={`border border-cyber-teal/20 rounded-lg p-6 ${isDark ? 'bg-muted-graphite' : 'bg-white'} shadow-md hover:shadow-cyber transition-shadow duration-300`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder="Enter 6-digit pincode (e.g., 600024)"
        ...
        <div className="flex items-end">
          <button
            ...
            disabled={isSearching || !pincode.trim() || !/^\d{6}$/.test(pincode)}
      ...
      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
        * Required. Consumes {apis.find(api => api.name === 'Pincode Population Search')?.default_credit_charge || 5.00} credits per query.
      </p>
      ...
      {searchError && (
        <div className={`p-4 rounded-lg border flex items-center space-x-3 ${isDark ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'} mb-6`}>
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              {searchError}
              {searchError === 'Pincode search service is currently unavailable' ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.inputPincode || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>District Population:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.populationDetails.districtPopulation || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Sub-District Population:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.populationDetails.subDistrictPopulation || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>State:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.locationDetails.state || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>District:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.locationDetails.district || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Sub-District:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.locationDetails.subDistrict || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Village Count:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.locationDetails.totalVillageCount || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Villages:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.locationDetails.villageList?.join(', ') || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Tier:</span>
                    <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {searchResults.tierCriteria.district || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-cyber-teal/20">
            <button
              onClick={() => {
                ...
                link.href = url;
                link.download = `pincode-population-${pincode}-${Date.now()}.json`;
                link.click();
...
export default PincodePopulationSearch;